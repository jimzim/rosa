...
package v1 // github.com/openshift-online/ocm-api-model/clientapi/accesstransparency/v1
		...
		type  AccessRequestStatusListBuilder struct {
			items []*AccessRequestStatusBuilder
		}
		...
		func (b *AccessRequestStatusListBuilder) Copy(list *AccessRequestStatusList) *AccessRequestStatusListBuilder {
			if list == nil || list.items == nil {
				...
				b.items = make([]*AccessRequestStatusBuilder, len(list.items))
				for i, v := range list.items {
					b.items[i] = NewAccessRequestStatus().Copy(v)
				}
			}
			return b
		}

		// Build creates a list of 'access_request_status' objects using the
		// configuration stored in the builder.
		func (b *AccessRequestStatusListBuilder) Build() (list *AccessRequestStatusList, err error) {
			items := make([]*AccessRequestStatus, len(b.items))
			...
			list = new(AccessRequestStatusList)
			list.items = items
			return
		}
