		...
		func WriteClusterRegistryConfig(object *ClusterRegistryConfig, stream *jsoniter.Stream) {
			...
			stream.WriteString(item)
				...
				WriteRegistryLocationList(object.allowedRegistriesForImport, stream)
						...
						present_ = len(object.fieldSet_) > 2 && object.fieldSet_[2] && object.imageDigestMirrorSets != nil
					if present_ {
						if count > 0 {
							stream.WriteMore()
						}
						stream.WriteObjectField("image_digest_mirror_sets")
				WriteImageDigestMirrorSetList(object.imageDigestMirrorSets, stream)
							count++
					}
						present_ = len(object.fieldSet_) > 3 && object.fieldSet_[3] && object.imageTagMirrorSets != nil
					if present_ {
						if count > 0 {
							stream.WriteMore()
						}
						stream.WriteObjectField("image_tag_mirror_sets")
				WriteImageTagMirrorSetList(object.imageTagMirrorSets, stream)
							count++
					}
						present_ = len(object.fieldSet_) > 4 && object.fieldSet_[4] && object.platformAllowlist != nil
					if present_ {
						if count > 0 {
							stream.WriteMore()
						}
			...
			WriteRegistryAllowlist(object.platformAllowlist, stream)
						...
						present_ = len(object.fieldSet_) > 5 && object.fieldSet_[5] && object.registrySources != nil
					if present_ {
						if count > 0 {
							stream.WriteMore()
						}
		...
		func ReadClusterRegistryConfig(iterator *jsoniter.Iterator) *ClusterRegistryConfig {
			...
			item := iterator.ReadString()
				...
				value := ReadRegistryLocationList(iterator)
					...
					case "image_digest_mirror_sets":
				value := ReadImageDigestMirrorSetList(iterator)
						object.imageDigestMirrorSets = value
						object.fieldSet_[2] = true
					case "image_tag_mirror_sets":
				value := ReadImageTagMirrorSetList(iterator)
						object.imageTagMirrorSets = value
