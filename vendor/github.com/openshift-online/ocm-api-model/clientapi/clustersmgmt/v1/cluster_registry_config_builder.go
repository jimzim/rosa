...
// 
		type  ClusterRegistryConfigBuilder struct {
			fieldSet_ []bool
					...
					imageDigestMirrorSets []*ImageDigestMirrorSetBuilder
					imageTagMirrorSets []*ImageTagMirrorSetBuilder
		...
		func (b *ClusterRegistryConfigBuilder) Empty() bool {
			if b == nil || len(b.fieldSet_) == 0 {
				...
				// ImageDigestMirrorSets sets the value of the 'image_digest_mirror_sets' attribute to the given values.
				//
				// 
						func (b *ClusterRegistryConfigBuilder) ImageDigestMirrorSets(values ...*ImageDigestMirrorSetBuilder) *ClusterRegistryConfigBuilder {
							if len(b.fieldSet_) == 0 {
								b.fieldSet_ = make([]bool, 6)
							}
							b.imageDigestMirrorSets = make([]*ImageDigestMirrorSetBuilder, len(values))
							copy(b.imageDigestMirrorSets, values)
							b.fieldSet_[2] = true
							return b
						}
			
				// ImageTagMirrorSets sets the value of the 'image_tag_mirror_sets' attribute to the given values.
				//
				// 
						func (b *ClusterRegistryConfigBuilder) ImageTagMirrorSets(values ...*ImageTagMirrorSetBuilder) *ClusterRegistryConfigBuilder {
							if len(b.fieldSet_) == 0 {
								b.fieldSet_ = make([]bool, 6)
							}
							b.imageTagMirrorSets = make([]*ImageTagMirrorSetBuilder, len(values))
							copy(b.imageTagMirrorSets, values)
							b.fieldSet_[3] = true
							return b
						}
		...
		func (b *ClusterRegistryConfigBuilder) Copy(object *ClusterRegistryConfig) *ClusterRegistryConfigBuilder {
			...
			if len(object.fieldSet_) > 0 {
				...
				copy(b.fieldSet_, object.fieldSet_)
					...
					if object.imageDigestMirrorSets != nil {
								b.imageDigestMirrorSets = make([]*ImageDigestMirrorSetBuilder, len(object.imageDigestMirrorSets))
								for i, v := range object.imageDigestMirrorSets {
									b.imageDigestMirrorSets[i] = NewImageDigestMirrorSet().Copy(v)
								}
					} else {
						b.imageDigestMirrorSets = nil
					}
					if object.imageTagMirrorSets != nil {
								b.imageTagMirrorSets = make([]*ImageTagMirrorSetBuilder, len(object.imageTagMirrorSets))
								for i, v := range object.imageTagMirrorSets {
									b.imageTagMirrorSets[i] = NewImageTagMirrorSet().Copy(v)
								}
					} else {
						b.imageTagMirrorSets = nil
					}
		...
		func (b *ClusterRegistryConfigBuilder) Build() (object *ClusterRegistryConfig, err error) {
			...
			if len(b.fieldSet_) > 0 {
				...
				copy(object.fieldSet_, b.fieldSet_)
					...
					if b.imageDigestMirrorSets != nil {
								object.imageDigestMirrorSets = make([]*ImageDigestMirrorSet, len(b.imageDigestMirrorSets))
								for i, v := range b.imageDigestMirrorSets {
									object.imageDigestMirrorSets[i], err = v.Build()
									if err != nil {
										return
									}
								}
					}
					if b.imageTagMirrorSets != nil {
								object.imageTagMirrorSets = make([]*ImageTagMirrorSet, len(b.imageTagMirrorSets))
								for i, v := range b.imageTagMirrorSets {
									object.imageTagMirrorSets[i], err = v.Build()
									if err != nil {
										return
									}
								}
