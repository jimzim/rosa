...
// starting with OpenShift 4.13+.
		...
		// NewImageTagMirrorSet creates a new builder of 'image_tag_mirror_set' objects.
		...
		func (b *ImageTagMirrorSetBuilder) Empty() bool {
			if b == nil || len(b.fieldSet_) == 0 {
				...
				//
				// 
						func (b *ImageTagMirrorSetBuilder) Mirrors(values ...*ImageMirrorBuilder) *ImageTagMirrorSetBuilder {
				...
				//
				// 
				func (b *ImageTagMirrorSetBuilder) Name(value string) *ImageTagMirrorSetBuilder {
					if len(b.fieldSet_) == 0 {
						b.fieldSet_ = make([]bool, 2)
					}
					b.name = value
		...
		func (b *ImageTagMirrorSetBuilder) Copy(object *ImageTagMirrorSet) *ImageTagMirrorSetBuilder {
			if object == nil {
				return b
			}
		...
		// Build creates a 'image_tag_mirror_set' object using the configuration stored in the builder.
		func (b *ImageTagMirrorSetBuilder) Build() (object *ImageTagMirrorSet, err error) {
			object = new(ImageTagMirrorSet)
			if len(b.fieldSet_) > 0 {
